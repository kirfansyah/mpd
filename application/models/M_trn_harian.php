...
class m_trn_harian extends CI_model{
	...
	function get_HasilHeader($idpekerjaan, $prefix){
		...
		$param = array(
			...
			'IDJamKerja' => $this->session->userdata($prefix.'_idjamkerja'),
		);
	...
	function get_hasil_parer($headerid, $NIK){
		$array = array('NIK' => $NIK);
		$this->db->select('TimbangWM, TimbangAK, HasilManual');
		$this->db->where($array);
	...
	function get_hasil_sheller($headerid, $NIK){
		$array = array('NIK' => $NIK);
		$this->db->select('TimbangWM, TimbangAK, HasilKC, HasilManual');
		$this->db->where($array);
		$this->db->from('fnMonitorHasilSheller('.$headerid.')');
		$query = $this->db->get();
		$result = $query->result();
		return $result;
	}

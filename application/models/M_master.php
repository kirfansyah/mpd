...
class m_master extends CI_Model{
	...
	function mst_line_group(){
		...
		$query = $this->db->query("
			select DISTINCT(GroupLine) from tblMstLine  where GroupLine like '%LL%'
				union ALL
			select GroupLine from tblMstLine  where GroupLine like '%SL%' 
		");
		return $query;
	}
	...
	function mst_sub_pekerjaan_mpd(){
		$sql = 'Select * From tblMstSubPekerjaan Where IDSubPekerjaan In(1,2)';
		return $this->db->query($sql);
	}

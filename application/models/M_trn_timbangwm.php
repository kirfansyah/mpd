...
class m_trn_timbangwm extends CI_Model{
	...
	function list_timbangwm(){
		// if ($this->session->userdata('accessalldata') === 1) {
			$sql = "Select top 500 TglJamMasuk, Tanggal, IDShift, JamKerja, IDLine, NamaLine, HasilHeaderID, "
	...
	function detail_timbangwm($param_mesin){
		$this->db->where($param_mesin);
		$this->db->limit(1);
		$this->db->order_by('HasilTimbangWMID','desc');
		$query = $this->db->get("tblOL_TrnHasilTimbangWhiteMeat");
		return $query->result();
	}

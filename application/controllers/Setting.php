...
class Setting extends CI_Controller{
	...
	function menu($aksi = null, $menuid = null){
		switch ($aksi) {
			case 'add':
				$this->menu_add();
				break;
			case 'edit':
				$this->menu_edit($menuid);
				break;
			case 'delete':
				$this->menu_delete();
				break;
	...
	//---- BEGIN MENU CONTROLLER ----//
		function menu_list(){
			...
			$data['record']=  $this->m_menu->list_menu()->result();
			...
			$this->template->display('setting/menu/menulist',$data);
		}
		...
		function menu_add(){
			...
			$data['falist'] = $arr;
			$data['menulist'] = $this->m_menu->list_menu_parent()->result();
			
			$data['message'] = $this->session->flashdata('pesansimpan');

			$this->template->display('setting/menu/menuadd',$data);
		}
		...
		function menu_edit($menuid){
			...
			$this->template->display('setting/menu/menuedit',$data);
		}
		...
		function menu_save(){
			...
			$menuid = $this->input->post('txtMenuID');
			if ($this->m_menu->cek_menuid($menuid) === 1){
				$pesan = pesan('ID Menu '.$menuid.' Sudah Ada',  pesan_error());
				$this->session->set_flashdata('pesansimpan', $pesan);
				redirect('setting/menu_add');
			}
			
			if ($this->m_menu->simpan($info) === 1){
				$pesan = pesan('Simpan Menu Berhasil.', 'success');
			} else {
				$err = mssql_get_last_message();
				$pesan = pesan('Simpan Menu Gagal.<br>Pesan : '.$err, 'error');
			}
		...
		function menu_update(){
			...
			if ($this->m_menu->update_menu($menuid, $info) === 1){
				$pesan = pesan('Update Menu Berhasil', 'success');
			} else {
				$err = mssql_get_last_message();
				$pesan = pesan('Update Menu Gagal.<br>Pesan : '.$err, 'error');
			}
		...
		function menu_delete(){
			$menuid = $this->input->post('menuid');
			$this->m_menu->delete_menu($menuid);
		}
	...
	//---- BEGIN AKSES CONTROLLER ----//
		function akses_list(){
			$data['grouplist'] = $this->db->get('tblOL_UtlGroupUsers')->result();
			$data['message'] = $this->session->flashdata('message');

			$this->template->display('setting/menuakses/akseslist',$data);
		}

		function akses_listmenu(){
			if('IS_AJAX') {
				$data['menulist'] = $this->m_menu->get_menu()->result();
				$this->load->view('setting/menuakses/listmenu',$data);
			}
		}
		...
		function akses_simpan(){
			...
			$this->session->set_flashdata("message",$pesan);
			redirect('setting/akses');
		}
	//---- END AKSES CONTROLLER ----//
}
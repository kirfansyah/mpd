...
class Laporan extends CI_Controller{
	...
	function clearbug(){
		...
		$mydate = date_create_from_format('d/m/Y',$d);
		if($mydate){
			$tglphp = $mydate->format('Y-m-d');
			$this->m_laporan->runfixbug($tglphp);
			echo json_encode(array('job'=>'Data berhasil di update!..klik refresh'));
		}else{
			echo json_encode(array('job'=>'Isi data tanggal'));
		}		

...
//				}
	...
	function rekap($pekerjaan){
		...
		if (!$this->input->post('txtHeaderID')){
			$info = array(
			...
			$hdrid = $this->m_absensi->insert_header_urut($info);
		} else {
	...
	function sheller($aksi = '', $nomormesin = 0){
		...
		switch ($aksi) {
			case 'set':
				...
				$headerHasilID 			= $this->m_trn_harian->get_HasilHeader(2, $prefix);
				...
				$anggota = $this->m_absensi->get_urutAnggota(2, $idline, $shift)->result();
				$data['listanggota'] = $anggota;
...
//				
			...
			case 'lembur' :
				...
				$nomormesin = $this->input->post('nomormesin');
				...
				$info = array('islembur' => $islembur				
							);							
				$issuccess = $this->m_absensi->update_detail_urut2($detailid,$info);
				if($issuccess){
					echo json_encode(array('status'=>1,'islembur'=>$islembur));
				}else{
					echo json_encode(array('status'=>0,'islembur'=>$islembur));
				}
                break;			
...
//				$data['cboperusahaan'] = $this->m_master->mst_perusahaan()->result();
				...
				$idSubPekerjaan			= $this->input->post('txtSubPekerjaan');
				$idshift				= $this->input->post('txtShift');
				$idline					= $this->input->post('txtLine');
//				$idperusahaan	= $this->input->post('txtPerusahaan');
				...
				if ($this->input->server('REQUEST_METHOD') == 'POST'){	
					...
					$data['search_result'] = $this->m_absensi->search_tk_sub($idpekerjaan, $idSubPekerjaan, $idshift, $idline, $idpemborong, $nama, $nik)->result();
...
//					);
					...
					// $data['selectsubpekerjaan'] = $idSubPekerjaan;
...
//					$data['idperusahaan'] = $idperusahaan;
					...
					$data['search_result'] 		= $this->m_absensi->search_tk_sub(2, '', '', '', '', '', '')->result();;
					$data['selectpekerjaan'] 	= 2;
					// $data['selectsubpekerjaan'] = 1;
	...
	function parer($aksi = '', $nomormesin = 0){		
		...
		switch ($aksi) {
			case 'set':
				...
				$anggota = $this->m_absensi->get_urutAnggota(3, $idline, $shift)->result();
				$data['listanggota'] = $anggota;
...
//				$idperusahaan = $this->input->post('txtPerusahaan');
				...
				if ($this->input->server('REQUEST_METHOD') === 'POST'){	
					...
					$data['search_result'] = $this->m_absensi->search_tk_sub(3, '', '', '', '', '', '')->result();
					$data['selectpekerjaan'] = 3;
					// $data['selectsubpekerjaan'] = 2;
					$data['idshift'] = $this->session->userdata('shl_shift');
					$data['idline']	= $idline;
//					$data['idperusahaan'] = $idperusahaan;
					$data['idpemborong'] = $idpemborong;

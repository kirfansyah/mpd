	...
	function do_changeline($halaman){
		...
		switch ($halaman) {
			...
			case 'whitemeat':
				...
				$sess_data['wmfrom_idline'] = $this->session->userdata('wm_idline');
				$this->session->set_userdata($sess_data);
			...
			case 'airkelapa':
				...
				$sess_data['akfrom_idline'] = $this->session->userdata('ak_idline');
				$this->session->set_userdata($sess_data);
			...
			case 'kelapacungkil':
				...
				$sess_data['kcfrom_idline'] = $this->session->userdata('kc_idline');
				$this->session->set_userdata($sess_data);
			...
			default:
				if (!$this->session->userdata('wm_idline') || !$this->session->userdata('wm_shift')){
					...
					if ($headerid === 0){	//Jika belum ada headernya, maka akan di-Insert
						...
						if ($this->session->userdata('groupid') == 6) {
					      ...
					      ,'IPAddr' => $ipaddr
							...
							$logtrnid = $this->m_trnhasil->insert_Log_trn($infoLog);
						}
					} else {
						if ($this->session->userdata('groupid') == 6 ){
							$userid = $this->session->userdata('userid');
							$cek_inspector = $this->m_trnhasil->get_inspector($headerid, 'WM');
							if ((strtoupper($cek_inspector['userid']) != strtoupper($userid))&&($cek_inspector['stselesai'] === 0)){
								if ($this->session->userdata('wmfrom_idline')){
									$from_line = $this->session->userdata('wmfrom_idline');
									$this->session->unset_userdata('wmfrom_idline');
									$this->session->set_userdata('wm_idline', $from_line);
									$pesan = pesan('Anda tidak bisa membuka transaksi Line '.$namaline.' shift '.$shift.', sedang di input '.$cek_inspector['username'],  pesan_error());
									$this->session->set_flashdata('message', $pesan);
									$this->timbang_whitemeat();
									break;
								}else {
									$pesan = pesan('Anda tidak bisa membuka transaksi Line '.$namaline.' shift '.$shift.', sedang di input '.$cek_inspector['username'],  pesan_error());
									$this->session->set_flashdata('message', $pesan);
							...
							} else if ($cek_inspector['stselesai'] != 0){
								if ($this->session->userdata('groupid') == 6) {
									$infoLog = array(
							      'HasilHeaderID' => $headerid
							      ,'UserID' => strtoupper($this->session->userdata('userid'))
					...
					if ($headerid === 0){	//Jika transaksi simpan header gagal, diarahkan ke index
						...
						$tgl_kerja = $this->session->userdata('serverdate');
						if ($tgl_kerja !== $tanggal){
							$pesan = pesan('Tanggal Transaksi <strong>('.$tanggal .')</strong> Bukan Tanggal Hari Ini<strong>('. $tgl_kerja.')</strong>,',  pesan_peringatan());
							$this->session->set_flashdata('savemsg', $pesan);
						}
...
//	TRANSAKSI HASIL - AIR KELAPA
			...
			default:
				if (!$this->session->userdata('ak_idline') || !$this->session->userdata('ak_shift')){
					...
					if ($headerid === 0){
						...
						if ($this->session->userdata('groupid') == 6 ){
							$userid = $this->session->userdata('userid');
							$cek_inspector = $this->m_trnhasil->get_inspector($headerid, 'AK');
							if ((strtoupper($cek_inspector['userid']) != strtoupper($userid))&&($cek_inspector['stselesai'] === 0 )){
								if ($this->session->userdata('akfrom_idline')){
									$from_line = $this->session->userdata('akfrom_idline');
									$this->session->unset_userdata('akfrom_idline');
									$this->session->set_userdata('ak_idline', $from_line);
									$pesan = pesan('Anda tidak bisa membuka transaksi Line '.$namaline.' shift '.$shift.', sedang di input '.$cek_inspector['username'],  pesan_error());
									$this->session->set_flashdata('message', $pesan);
									$this->timbang_airkelapa();
									break;
								}else {
									$pesan = pesan('Anda tidak bisa membuka transaksi Line '.$namaline.' shift '.$shift.', sedang di input '.$cek_inspector['username'],  pesan_error());
									$this->session->set_flashdata('message', $pesan);
	...
	function simpan_timbangak($no_mesin){
		if (isset($_POST['SimpanMesin'.$no_mesin])){
			...
			$isvalid = $this->m_trnhasil->is_valid_line($this->session->userdata('ak_idline'),$no_mesin);
			if(!$isvalid){				
				return;
			}
			...
			default:
				if (!$this->session->userdata('kc_idline') || !$this->session->userdata('kc_shift')){
					...
					if ($headerid === 0){
						...
						if ($this->session->userdata('groupid') == 6 ){
							$userid = $this->session->userdata('userid');
							$cek_inspector = $this->m_trnhasil->get_inspector($headerid, 'KC');
							if ((strtoupper($cek_inspector['userid']) != strtoupper($userid))&&($cek_inspector['stselesai'] === 0)){
								if ($this->session->userdata('kcfrom_idline')){
									$from_line = $this->session->userdata('kcfrom_idline');
									$this->session->unset_userdata('kcfrom_idline');
									$this->session->set_userdata('kc_idline', $from_line);
									$pesan = pesan('Anda tidak bisa membuka transaksi Line '.$idline.' shift '.$shift.', sedang di input '.$cek_inspector['username'],  pesan_error());
									$this->session->set_flashdata('message', $pesan);
									$this->timbang_airkelapa();
									break;
								}else {

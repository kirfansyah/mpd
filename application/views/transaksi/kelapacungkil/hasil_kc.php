...
<div class="page-content">
	<h3 class="header smaller lighter blue">		
		<a href="<?php echo site_url('transaksi/proses_hasilkc_selesai');?>" class="btn btn-danger bolder">
			<i class="icon-ok"></i> Selesai
		</a>
		<a href="<?php echo site_url('monitor/kelapacungkil');?>" class="btn btn-primary bolder">
			<i class="icon-table"></i> Buka Monitor Kelapa Cungkil
		</a>
		<!-- <a href="<?php //echo site_url('transaksi/pindahline/kelapacungkil');?>" class="btn btn-success bolder"
	...
	<h3 class="header smaller lighter blue">
		<div class="row-fluid-fluid">
			<?php
				echo form_open('transaksi/do_changeline/kelapacungkil', array('id'=>'formPindah' ,'class'=>'form-horizontal'));
				foreach ($line as $value) {
				if ($value->NamaLine == $namaline) {$disable = 'disabled'; $btn = 'btn-danger';}
				else {$disable = ''; $btn = 'btn-success';}
			?>
			<button class="btn bolder btn-small <?php echo $btn ?>" type="submit" name="txtLine" value="<?php echo $value->IDLine?>" <?php echo $disable ?> style="width: 80px; margin: 2px;">
				<i class="icon-hdd"></i>
				<?php echo $value->NamaLine?>
			</button>
			<?php } echo form_close();?>
		</div>
	</h3>
	<div class="row-fluid">
		<div class="span12">
			<div class="judul">
				Hasil Kelapa Cungkil <?php echo $namaline;?> <span class="badge badge-success"> ID Transaksi : <?php echo $headerid;?> </span>
			</div>
			
			<div class="trx-user-info trx-user-info-striped">
				<div class="trx-info-row">
					<div class="trx-info-name"> Jenis Produk </div>
					<div class="trx-info-value"> KC </div>
				</div>
				<div class="trx-info-row">
				...
				<div class="trx-info-row">
					...
					<div class="trx-info-value"><?php echo $this->m_trnhasil->get_inspector($headerid, 'KC')['username'];?></div>
				</div>
			</div>
						...
						<div class="tab-content">
							<?php				
								for ($i = $nomesinawal; $i <= $nomesinakhir; $i++){
									...
									// echo form_open('transaksi/simpan_hasilkc/'.$i, array('id'=>'formhasilkc'.$i ,'class'=>'form-horizontal', 'name'=>'mesin'.$i));
									echo form_fieldset();
								...
								<table id="tabelMesin<?php echo $i;?>" class="table table-bordered table-condensed">
									<thead>
										<tr>
											...
											<th class="center middle">Kelapa Cungkil</th>
									...
									<tbody>
										<?php
											...
											$detail_mesin = $this->db->get_where('vwOL_TrnHasilKelapaCungkil', $param_mesin);
											...
											if ($detail_jumlah > 0) {
												foreach ($detail_mesin->result() as $dm) {
													...
													$totHasilKC = $totHasilKC + $dm->HasilKC;
													
													echo '<tr class="bigger-120">';
													echo '<td class="center middle">'.$j.'</td>';
													echo '<td class="center middle">'.floatval($dm->HasilKC).'</td>';
													echo '<td class="center middle">
															<a href="kelapacungkil/edit/'.$dm->HasilKCID.'" class="btn btn-medium btn-warning"
																data-toggle="remote-modal" data-target="#modaledit" title="Edit">
															<i class="icon-pencil bigger-120"></i>
															</a>
															
															<button id="btnDelete" detailid="'.$dm->HasilKCID.'" nomormesin="'.$i.'" 
									...
									<tfoot>
										<tr class="bigger-120">
											...
											<td class="center middle">
												<input type="number" min="0" name="txtHasilKC<?php echo $i;?>" placeholder='0' class="input input-small text-right numeric" id="txtHasilKC<?php echo $i;?>"/> 
											</td>
										...
										<tr class="bigger-140 red" id="totalmesin<?php echo $i;?>">
											...
											<td class="center middle" id="totalTimbang"><?php echo $totHasilKC;?></td>
											<td class="center middle">&nbsp;</td>
										</tr>
									</tfoot>
								</table>
...
<script type="text/javascript">
 ...
 //    $(document).ajaxStop($.unblockUI);
	...
	$(document).ready(function() {
		...
		$('.btnSimpan').click(function(){
			...
			var dataString = 
							...
							"&txtHasilKC"+nomorMesin+"="+$('#txtHasilKC'+nomorMesin).val()
							
			;
			$.ajax({
				url 		: "<?php echo site_url('transaksi/simpan_hasilkc/')?>/"+nomorMesin,
				type 		: "POST",
				data 		: dataString,
				cache		: false,
				beforeSend  : function(){
	...
	function add_data_table(nomorMesin){
		...
		$.ajax({
			url 		: "<?php echo site_url('transaksi/detail_hasilkc');?>/"+nomorMesin,
			type		: "post",
			...
			success 	: function(info){
				$(tabelMesin+' tbody').append('<tr class="bigger-120">'+
					...
					'<td class="center middle">'+parseFloat(info[0].HasilKC)+'</td>'+
					'<td class="center middle">'+
						'<a href="kelapacungkil/edit/'+info[0].HasilKCID+'" class="btn btn-medium btn-warning" '+
							'data-toggle="remote-modal" data-target="#modaledit" title="Edit">'+
							'<i class="icon-pencil bigger-120"></i>'+
						'</a> '+
														
						'<button id="btnDelete" detailid="'+info[0].HasilKCID+'" nomormesin="'+nomorMesin+'" '+ 
							'class="btn btn-medium btn-danger hapus" title="Hapus">'+
							'<i class="icon-trash bigger-120"></i>'+
						'</a>'+
					'</td>'+
				'</tr>');
				totalTimbang 	= totalTimbang + parseFloat(info[0].HasilKC);				
				$('#totalmesin'+nomorMesin+' >#totalTimbang').text(totalTimbang.toFixed(2));
				$('#txtHasilKC'+nomorMesin).val(null);
				$(tabelMesin+' >tfoot >tr:first').find('td:first-child').text(rowcount + 1);
			},
			error 		: function(){
				// alert('koneksi gagal');
				return location.href = "<? echo site_url('transaksi/kelapacungkil');?>";
			}
		});
	...
	$(document).on('click', '.hapus', function() {
		// $('.hapus').click(function(){
			...
			bootbox.confirm('Hapus data mesin '+nomormesin+'?',function(result){
				if (result){
					$.ajax({
						url:"<?php echo site_url('transaksi/hapus_hasilkc');?>",
						type:"POST",
						data:"detailid="+detailid,
						cache:false,
						success:function(){
							return location.href = "<?php echo site_url('transaksi/hapus_hasilkc/berhasil');?>/"+nomormesin;
						},
						error:function(){
							return location.href="<?php echo site_url('transaksi/hapus_hasilkc/gagal');?>/"+nomormesin;
						}
					});
				}else{

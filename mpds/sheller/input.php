				...
				<div class="span12">
					<div class="tabbable" >
						<ul class="nav nav-tabs" id="tababsensi">
							<li class="<?php if (!$this->session->userdata('shl_daftar')) echo 'active';?>">
						...
						<div class="tab-content">
							<div id="aturmesin" class="tab-pane <?php if (!$this->session->userdata('shl_daftar')) echo 'in active';?>">
								<?php
								for ($i = $nomesinawal; $i <= $nomesinakhir; $i++){
									...
									if ($nama === ''){
										...
										?>	  
										    <div class="tools">												
												<div class="action-buttons">												    
													<a href="#" islembur="<?=$islembur?>" detailid="<?=$urutdtlid?>" nomormesin="<?=$i?>" namatk="<?=$nama?>" 
													   class="btn btn-mini btn-primary btn-no-border lembur" title="Extra-Shift">
														<i class="<?=$islembur==1 ? 'icon-check' : 'icon-check-empty'?> bigger-150"></i>
													</a>												
												</div>
												<div class="action-buttons">
							...
							<div id="anggota" class="tab-pane <?php if ($this->session->userdata('shl_daftar')) echo 'in active';?>">
								<?php 
									echo form_open('daftar/rekap/sheller', array('id'=>'formDaftarSheller', 'class'=>'form-horizontal'));
									echo form_fieldset();
									if ($this->session->userdata('shl_daftar')) $hidden = '';
									else $hidden = 'hidden';
								?>
								...
								<div class="widget-box transparent">
									<div class="widget-header widget-header-flat">
										...
										<a href="<?php echo site_url('daftar/sheller/end');?>" id="btnSimpanAnggota"
													...
													<table class="table table-bordered table-condensed">
														...
														<tbody>
															...
															for ($i = 1; $i <= 20; $i++){
																...
																if ($transid != 0){
																	echo '
																	<a href="'.site_url('/daftar/sheller/search/'.$i.'/ganti_isi/'.$transid).'" class="btn btn-warning btn-small"><i class="icon-edit"></i>Ganti</a>
																	...
																	echo '
																	<a href="'.site_url('/daftar/sheller/search/'.$i.'/ganti_kosong').'" class="btn btn-primary btn-small"><i class="icon-search"></i>Tambah dan Cari</a>';
																}
																echo '</div></td>';
	...
	$('.lembur').click(function(){
		...
		bootbox.confirm(msg,function(result){
			if(result){
				...
				$.ajax({
					url:"<?= site_url('absensi/sheller') ?>/lembur/"+nomormesin,
					type:'POST',
...
//	menampilkan modal
	...
	$('#btnUbahAnggota').click(function(){
		...
		console.log(tgl_skrg);
		var hari = tgl_skrg.getDate();
		console.log(hari);
		// if ((hari != 1)||hari != 16){
			// bootbox.alert('Pendafataran Ulang tidak Dapat Dilakukan selain Tanggal <strong>1 atau 16</strong>');
		// } else {
